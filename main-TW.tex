...
Let $E$ be a CI and $S$ be a support set for $E$. We denote with $dts(E,S)$ the minimum size of a DT for $E$ that uses exactly all the features in $S$ and with $dts(E)$ the minimum size of a DT for $E$, that is $dts(E)=min_S \{dts(E,S)\}$.

\begin{theorem}
For every integer $k\geq 1$, there is a CI $E$ with $2^k$ examples such that for every minimal support $S$, the multiplicative gap between $dts(E,S)$ and $dts(E)$ is at least $\frac{2^{k+2}-1}{2k+5}$.
\end{theorem}

\begin{proof}
Let $k\geq 1$ be an integer and let $E=E_k$. By construction $|E_k|=2^k$ and by Lemma~\ref{lem:umss}, $S_k$ is the only minimal support set for $E_k$. By Lemma~\ref{lem:omss}, we have that $dts(E_k,S_k)=2^{k+2}-1$. 
Moreover, since by Lemma~\ref{lem:emss} $T_k$ is a DT for $E_k$, we have that $dts(E_k)\leq dts(E_k,S^*_k)\leq |T_k|=2k+5$.
In conclusion we have that $$dts(E_k,S_k)\geq \frac{2^{k+2}-1}{2k+5} dts(E_k).$$ %\approx \frac{2^{k+1}}{k}
\end{proof}
...
Let $E$ be a CI and $S$ be a support set for $E$. We denote with $dth(E,S)$ the minimum height of a DT for $E$ that uses exactly all the features in $S$ and with $dth(E)$ the minimumsheight of a DT for $E$, that is $dth(E)=min_S \{dth(E,S)\}$.

\begin{theorem}
For every integer $k\geq 1$, there is a CI $E$ with $2^k$ examples such that for every minimal support $S$, the multiplicative gap between $dth(E,S)$ and $dth(E)$ is at least $\frac{k+1}{log(k)+2}$.
\end{theorem}

\begin{proof}
Let $k\geq 1$ be an integer and let $E=E_k$. By construction $|E_k|=2^k$ and by Lemma~\ref{lem:umss}, $S_k$ is the only minimal support set for $E_k$. By Lemma~\ref{lem:omss}, we have that $dth(E_k,S_k)=k+1$. Moreover, since by Lemma~\ref{lem:hemss} $T^k_*$ is a DT for $E_k$, we have that $dth(E_k)\leq dth(E_k,S'_k)\leq |T^k_*|=log(k)+2$.
In conclusion we have that $$dth(E_k,S_k)\geq \frac{k+1}{log(k)+2} dth(E_k).$$ %\approx \frac{2^{k+1}}{k}
\end{proof}
