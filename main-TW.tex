...
examples in $E_T(t)$, go the left (right) child of $t$ in $T$. Therefore,
...
in $\feat(b)$ with label $l$. Intuitively, DT skeletons are
...
Let $T$ be a DT skeleton and $t \in V(T)$. Similarily as we
...
\feat(\anc^L(t))$ ($f_l \in \feat(\anc^R(t))$). Similarily, as for
...
\begin{proof}
  ...
  left (right) redundant in $T$. If $\feat(t')$ is a real or forgotton
...
\begin{proof}
  ...
  feature $f_l$, then $t$ is left (right) redundant
  because of some left (right) ancestor $t'$ with
  $\feat(t')=\feat(t)=f_l$. Moreover, because $e$ goes to $t$, $e$ went to
  the left (right) child of $t'$ and therefore $e$ goes to the
...
\begin{observation}\label{obs:DTSsize}
  Let $T$ be a reduced DT skeleton whose forgotton features use a set
...
\begin{proof}
  ...
  features. Similarily, $P$ can contain at most $k_I$ future features,
...
\begin{observation}\label{obs:sizeofreduced}
  The are at most $\tfDTsenumE$ reduced DT skeletons whose forgotton features use a set
  of at most $k_F$ labels and whose future features use a set of at
...
(usually $L=[k]$).
A \emph{feature relabeling} is a function
...
tree obtained after relabeling all features used by $T$ according to
...
We now show an important property on the interchangebility of feature relabelings and
...
\begin{lemma}\label{lem:relabRed}
  ...
  $t\in V(T)$ is left (right) redudant in $T$, then it is also
...
\begin{proof}
  ...
  which exists because $(T_L,s_L)$ is semi-valid for $b_L$. Similarily, let
  ...
  applied to $T_H'$ to obtain $T_H$ for every $H \in \{L,B\}$. To do so we
  first need to introduce the so-called plugin operation and notions around this
  operation. We will do so in the next two
  paragraphs.
  
  Let $D$ and $D'$ be two DT (templates/skeletons). Let $P=(d,p_1,\dotsc,p_\ell,d')$ be the path
  from $d$ to $d'$ in $D$ such that $d$ is an ancestor of $d'$ in
  $D$, for some integer $\ell$. Moreover, let $e=(p,c)$ be an edge in $D'$ such that $p$ is the
  parent of $c$ in $D'$. We say that the DT (template/skeleton) $D''$
  is obtained by \emph{pluging in the path $P$ into $D'$ at edge $e$} if $D''$
  is obtained from $D'$ by doing the following. For an inner vertex
  $p_i$ of $P$, let $D(P,p_i)$ be the subtree of $D$ rooted at the
  unique child $c$ of $p_i$ that is not on $P$. Let $P'$ be the induced subtree of $D$
  containing all vertices of $P$ plus all vertices of $D(P,p_i)$ for
  every $i$ with $1 \leq i \leq \ell$. Then, $D''$ is obtained from
  $D'$ by removing the edge $e=(p,c)$, adding $P'$, and adding the edge
  from $p$ to $p_1$ as well as the edge from $p_\ell$ to
  $c$. Moreover, $D''$ inherents all feature assignments as well as
  the left (right) child relation from $D$ and $D'$.

  The significance
  of the plugin operation comes from the fact that it allows us to
  reverse the reduction that has been applied to a DT
  (template/skeleton). For instance, consider $T_H'$ and $T_H$. Then,
  $T_H=\red(\rlstan{b}(T_H')$ and we can use the plugin operation to
  obtain $T_H'$ from $T_H$ as follows. Let $z_H : V(T_H) \rightarrow
  V(T_H')$ be the injective function mapping every
  node in $T_H$ to its original node in $T_H'$. Then, we first use
  $z_H$ to reverse the relabelling given by $\rlstan{b}(T_H')$, i.e.,
  let $T_H^0$ be the DT template obtained from $T_H$ by setting
  $\feat_{T_H^0}(t)=\feat_{T_H'}(z_H(t))$ for every $t \in V(T_H^0)$. We now add back the nodes in
  $V(T_H')\setminus V(T_H)$ with the help of our plugin operation. In
  particular, for every edge $e=(p,c)$ in $T_H^0$, where $p$ is the
  parent of $c$ in $T_H^0$, let $P(e)$ be the path in $T_H'$ between
  $z_H(p)$ and $z_H(c)$. Let $T_H^1$ be the DT template obtained from
  $T_H^0$ after plugin in the path $P(e)$ into $T_H^0$ at edge $e$,
  for every edge $e=(p,c)$ of $T_H^0$. Then, it is easy to see that $T_H^1=T_H'$.
  We will now use the plugin operation to obtain $T'$ from $T_L'$,
  $T_R'$, $T_L$, and $T_R$ in a very similar manner.
  ...
  $e$ is correctly classified by $T_H'$. Let $l$ be the leaf in $T_H'$
  that contains $e$, i.e., $e \in E_{T_H'}(l)$ and let $Q$ be the path
  from the root of $T_H'$ to $l$. Then, $l$ also exists
  in $T'$ and moreover the path $P$ from the root of $T'$ to $l$
  ...
  along the path $P$ in $T'$ and therefore ends up in $l$, which shows
  ...
  child of $t$. Finally, since $P$ contains $l$ it follows that $P$
  ...
  $\rlstan{b_H}(T_H')$. Therefore, $\bigcup_{H \in
    \{L,R\}}V(T_H')\setminus V(T_H)$ is the set of all redudant nodes
