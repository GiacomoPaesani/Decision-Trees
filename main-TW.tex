...
The features in $\{f_1,\ldots,f_k\}$ form the unique minimal support set in $S=\{f_1,\ldots,f_k,f^*\}$ for $E_k$.
\end{lemma}
...
In particular node that $T_{\sigma,\tau}$ has $2k+5$ nodes ($k+2$ of those are inner nodes).
...
\smallskip
In order to prove the backward direction, we assume that $T'=T_{\sigma,\tau}$, for some permutation $\sigma$ and $\tau$. By construction, $r$, and its feature $f^*$, sends every negative example to its left child $c_\ell$, which is a negative leaf, except for the two negative examples, that is, if $\{e_1^-,e_2^-\}=E^-_k\cap D_k$ then $E_{T'}(c_\ell)=E^-_k\setminus \{e_1^-,e_2^-\}$ and $E_{T'}(v_1)=E^+_k\cup \{e_1^-,e_2^-\}$.
