...
\subsection{For Size}
Let $D_k$ be the set of all the examples $e$ in $E_k$ such that $f_i(e)=1$ for every $i\in [k-2]$ and denote by $\overline{D_k}$ the set $E_k\setminus D_k$. Now we are ready to define a new feature $f^*$ as follows: $f^*(e)=1$ if either $e$ is a positive example or $e\in D_k$ and $f^*(e)=0$ otherwise.
For simplicity, the sets $S_k^*$ denotes $\{f_1,\ldots,f_k,f^*\}$. 
...
\node at (4.5,-4) {$+$};
\end{tikzpicture}
\caption{The CI $E_3$ and the DT $T_3$.}
\end{minipage}
%\label{fig:example}
\end{figure}
...
We have that $\frac{dts(E_k,S_k)}{dts(E_k,S^*_k)}\geq \frac{2^{k+2}+1}{2k+5}\approx \frac{2^{k+1}}{k}$.
...
For every integer $k\geq 1$, let us describe a DT $T^k_*$ as follows. The root of $r$ has feature $f'$ and its left branch is the DT $T^k$. The right child of $r$ node $u$ with feature $f''$. The left/right child of $u$ is a negative/positive leaf. In particular note that $T^k_*$ has height $log(k)+2$.
...
\end{tikzpicture}
\end{minipage}
\caption{The CI $E_3$, the DT $T^3$ and the DT $T^3_*$} %\label{fig:example}
\end{figure}
...
We have that $\frac{dth(E_k,S_k)}{dth(E_k,S'_k)}\geq \frac{k+1}{log(k)+2}\approx \frac{k}{log(k)}$.
