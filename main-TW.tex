...
Let $T$ be a pseudo DT and $S$ be a support set for $E$. An {\it $S$-feature assigment} of $T$ is a mapping from a subset of the non-leaf nodes of $T$ to $S$. 
For every $v\in V(T)$, we define the set of {\it expected examples} $E_v$ as follows:

\begin{itemize}
\item if $v$ is the root, then $E_v=E$;
\item if $v$ is the left child of a supported feature node $v_p$ with feature $f_p\in S$, then $E_v=E_{v_p}[f_p\leq th_L(v_p)+1]$;
\item if $v$ is the right child of a supported feature node $v_p$ with feature $f_p\in S$, then $E_v=E_{v_p}[f_p> th_R(v_p)-1]$;
\item if $v$ is a child of an unknown feature node $v_p$, then $E_v=E_{v_p}$.
\end{itemize}

Node that the definition of $E_v$ is strictly related with the following: if $v$ has a supported feature $f$, let $c_{\ell}$ and $c_r$ be the left, risp. right, child of $v$, we define two values $th_L(t)$ and $th_R(T)$ as follows:

\begin{itemize}
\item let $th_L(v)$ be the maximum value in $D_E(f)$ such that every example in $E_v[f\leq th_L(v)]$ is correctly classified in $T_{c_{\ell}}$;
\item let $th_R(v)$ be the minimum value in $D_E(f)$ such that every example in $E_v[f> th_R(v)]$ is correctly classified in $T_{c_r}$.
\end{itemize}
