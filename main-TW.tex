...
In the following, let $E$ be a CI and $q\not\in feat(E)$. A {\it pattern} is a pair $(T,\phi)$ where $T=(V(T),A(T))$ is a rooted subcubic tree, every leaf-node is either a {\it positive} or {\it negative} leaf and $\phi$ maps every inner-node $v\in V(T)$ to an element of $feat(E)\cup \{q\}$. We say that $\phi$ is the {\it trait} function of $T$ and $\phi(v)$ is the trait of node $v\in V(T)$.
...
A pattern $(T,\phi^*)$ is an {\it improvement} for a pattern $(T,\phi)$ if $\phi^*(v)=\phi(v)$ for every $v$ such that $\phi(v)\in feat(E)$. 
A {\it complete improvement} $(T,\phi^*)$ for $(T,\phi)$ is an improvement such that $Im(\phi^*)\subseteq feat(E)$. 
We say that $(T,\phi)$ can classify $E$ if there is a complete improvement $(T,\phi^*)$ for $(T,\phi)$ which can be extended to a DT for $E$.
...
\item if $v$ is the left child of a node $v_p$ such that $\phi(v_p)\in feat(E)$, then $E_v=E_{v_p}[\phi(v_p)\leq th_L(v_p)+1]$;
\item if $v$ is the right child of a node $v_p$ such that $\phi(v_p)\in feat(E)$, then $E_v=E_{v_p}[\phi(v_p)> th_R(v_p)-1]$;
\item if $v$ is a child of a $v_p$ such that $\phi(v_p)=q$, then $E_v=E_{v_p}$.
\end{itemize}

Node that the definition of $E_v$ is strictly related with the following: if $\phi(v)\in feat(E)$, let $c_{\ell}$ and $c_r$ be the left, risp. right, child of $v$, we define two values $th_L(v)$ and $th_R(v)$ as follows:

\begin{itemize}
\item let $th_L(v)$ be the maximum value in $D_E(\phi(v))$ such that every example in $E_v[\phi(v)\leq th_L(v)]$ is correctly classified in $T_{c_{\ell}}$;
\item let $th_R(v)$ be the minimum value in $D_E(\phi(v))$ such that every example in $E_v[\phi(v)> th_R(v)]$ is correctly classified in $T_{c_r}$.
\end{itemize}

Before formally proving in Lemma~\ref{lem:LR} that we are able to compute $E_v$ and $th_L(v)$, $th_R(v)$ (when $v$ has a supported feature) for every $v\in V(T)$, we want to describe the role of $E_v$ in the proof of Lemma~\ref{lem:comp-branch}.
...
The first step would be to compute a minimum  size DT $T^*$ for $E$ such that $feat(T^*)=S$ (note this can be done by Theorem~\ref{lem:dtthres}).
Next we analyse the case an optimal DT for $E$ uses not only every feature from $S$ but some additional 
feature: for this reason we consider partial DTs $T$ of size at most $s$ such that the set of supported features of $T$ is exactly $S$.

To understand the properties of an unknown feature of $T$, we suppose $u\in V(T)$ has an unknown feature that does not distinguish any pair of examples.
Clearly $T$ can be extended to a DT for $E$ only if the smaller $T'$, obtained from $T$ by removing $u$ and the subtree of $T_u$ that receives no example, can. So let us suppose $T$ can not be extended to a DT for $E$. 

\medskip
\noindent
{\bf Claim 1.} {\it For every node $v\in V(T)$, the set $E_v$ is a set of examples such that $T_v$ can not be extended to a DT for $E$.}
