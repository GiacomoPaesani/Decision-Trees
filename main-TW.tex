...
\begin{abstract}
  ...
  from the CIs rank-width decomposition.  The key to the algorithm is a
...
integer $s$, deciding whether a $E$ has a decision tree with at most
...
bounded rank-width and unbounded treewidth. We denote the rank-with
...
Next, we argue that rank-width is a particularly well-suited and general
choice as a parameter for this problem because of its robustness and
generality.  Borrowing from a similar notion for propositional CNF
formulas \cite{Lewis78}, we define a \emph{renaming} $r_X(E)$ of a CI
$E$ with respect to a set $X\subseteq F(E)$ to be the CI containing
all the examples $e_X$ for $e\in E$ with $e_X(f)=1-e(f)$ if $f\in X$
and $e_X(f)=e(f)$ if $f\notin X$. Clearly, the size of a smallest
decision tree is the same for $E$ and all its renamings.  Let is
define the \emph{renamable rank-width} of $E$ as
$\rw^*(G(E))=\min_{X\subseteq F(E)}\rw(r_X(G(E)))$ and the
\emph{renamable treewidth} of $E$ as
$\tw^*(G(E))=\min_{X\subseteq F(E)}\tw(r_X(G(E)))$.  For two
...
each other, they are \emph{domination equivalent}.
%If neither of them
%dominates the other, they are \emph{domination orthogonal}.

\begin{enumerate}\setcounter{enumi}{1}\em 
...
Result~\reflip{rw-rw*} follows ... \todo[inline]{give
  here references or briefly sketch proofs for the three results}
...
While the DP approach using treewidth is well understood and can often
...
which the tree decomposition is given), the same DP approach can
become rather involved if applied to problems whose solutions have no
or only minor resemblence to the graph for which one is given a tree
...
An {\it example} $e$ is a function $e:\feat(e) \rightarrow \Z$ defined
on a finite set $\feat(e)$ of {\it features}, where each feature $f$ comes
with a possibly infinite linearly ordered domain $\dom(f) \subseteq \Z$,
which we assume to be, w.l.o.g., a subset of the integers. For a set $E$ of
...
if $X\subseteq E^+$ or $X \subseteq E^-$; otherwise $X$ is {\it
  non-uniform}. We say that a CI $E$ is binary if all features in
$\feat(E)$ are \emph{binary}, i.e., $e(f) \in \{0,1\}$ for every $f
\in \feat(e)$.
...
We define the {\it incidence graph} of $E$, denoted by $G_I(E)$, as
...
\item $t$ is labeled with an integer \emph{threshold} denoted by $\thr_T(t)$ or simply
  $\thr(t)$ if $T$ is clear from the context,
...
Let $E$ be a CI and let $T$ be a DT for $E$, i.e., $T$ is a DT with $\feat(T)\subseteq \feat(E)$.
For each node $t$ of $T$ we define
$E_T(t)$ as the set of all examples $e\in E$ such that for every left
(right) ancestor $t_A$ of $t$ in $T$, it holds that $e(\feat(t_A))\leq
\thr(t_A)$ ($e(\feat(t_A))> \thr(t_A)$). $T$ \emph{classifies} an example $e\in E$ if $e$ is a positive (negative)
example and $e\in E_T(l)$ for a positive (negative) leaf $l$ of
$T$. We say that $T$ \emph{classifies} $E$ if $T$
...
We will consider the following decision problems.

\pbDef{\probfont{Minimum Decision Tree Size} (\DTL)}{A CI $E$ and an
  integer $s$.}{Is there a DT of size at most $s$ that
  classifies $E$?}

\pbDef{\probfont{Minimum Decision Tree Depth} (\DTLh)}{A classification
  instance $E$ and an integer $h$.}{Is there a decision
  tree of depth at most $h$ that classifies $E$?}

\newcommand{\DTnass}{\tau}
In Section~\ref{sec:twfpt}, we will only deal with binary CIs $E$. In this
case, we can assume that $\thr(t)=0$ for every inner node $t$ of a DT
for $E$ and we will therefore omit the threshold function for
simplicity. Moreover, in this case, we denote by $\DTnass(t)$ for every $t \in V(T)$, the partial feature
assignment $\alpha : \feat(\anc(t)) \rightarrow \{0,1\}$ given by
$\alpha(f)=0$ if $f \in \feat(\anc^L(t))$ and $\alpha(f)=1$ if $f \in \feat(\anc^R(t))$.


We now give some simple auxiliary lemmas that are required by our algorithms.

\begin{lemma}\label{lem:enum-dt-fund}
  Let $A$ be a set of binary features of size $a$. Then the number of DTs of
...
\newcommand{\QF}{?}
\newcommand{\fixn}{\textsf{FIX}}

Let $E$ be a CI. We denote by $\QF$ a new feature, which we call the \emph{unknown feature},
i.e., $\QF \notin \feat(E)$. A \emph{decision tree pattern} is a DT
$T$ without thresholds that is allowed to use the unknown feature feature, i.e.,
$\feat(T)\subseteq \feat(E) \cup \{\QF\}$. We say that an inner node
$t$ of $T$ is \emph{fixed} if $\feat(t) \in \feat(E)$ and
\emph{non-fixed (or unknown)} otherwise.  A DT pattern $T'$ is an \emph{improvement}
of a DT pattern $T$ if $T=T'$ and $\feat_{T'}(t)=\feat_T{t}$ for every
fixed node $t$ of $T$. We say that $T'$ is \emph{complete} if
$\feat(T') \subseteq \feat(E)$. A \emph{threshold assignment} for a DT
pattern $T$ is a function $\thr$ that provides a threshold assignment for
every fixed node of $T$, i.e., $\th : \fixn(T) \rightarrow \Z$, where
$\fixn(T)$ is the set of all fixed nodes of $T$.
%, such that $\thr\in \dom(\feat(t))$.
Note that any complete DT pattern $T$ represents a DT.

Note that we assume that if $t$ is a node of a DT pattern with
$\feat(t)=\QF$, then any example that ends up in $t$ is send to both
its left and its right child in $T$. In particular, we generalize
$E_T(t)$ to DT patterns $T$ with a threshold assignment $\thr$ by
setting $E_T(t)$ to be the set of all examples $e \in E$ with such
that for every left
(right) ancestor $t_A$ of $t$ in $T$, it holds that either
$\feat(t_A)=\QF$ or $e(\feat(t_A))\leq
\thr(t_A)$ ($e(\feat(t_A))> \thr(t_A)$).

A DT pattern $T$ is \emph{valid} for a set of examples $E'\subseteq E$
if there is threshold assignment $\thr$ (for the fixed nodes) such that the
DT represented by $T$ and $\thr$ classifies $E'$.
...
\newcommand{\exex}{\textsf{EXP}}
Let $E$ be a CI and $T$ be a DT pattern for $E$.
For every $t\in V(T)$, we define the set of {\it expected examples}
$\exex_t$ recursively as follows:
\note{define examples restricted to an assignment, i.e., notation E[]}
\begin{itemize}
\item if $t$ is the root of $T$, then $\exex_t=E$;
\item if $t$ is the left child of a fixed node $t'$, then $\exex_t=\exex_{t'}[\feat(t')\leq \thr^L(t')+1]$;
\item if $t$ is the right child of a fixed node $t'$, then $\exex_{t}=\exex_{t'}[\feat(t')> \thr^R(t')-1]$;
\item if $t$ is a child of a non-fixed node $t'$, then $\exex_t=\exex_{t'}$.
\end{itemize}
Note that the definition of $\exex_t$ requires the definition of the
\emph{left (right) threshold}, denote by $\thr^L(t)$ ($\thr^R(t)$),
which for a given fixed node $t$ of $T$ with left (right) child $c_l$
($c_r$) are defined as follows:
\begin{itemize}
\item $\thr^L(t)$ is the maximum value in $\dom(\feat(t))$ such that $T_{c_{l}}$ is valid for $\exex_t[\feat(t)\leq \thr^L(t)]$;
\item $\thr^R(t)$ is the minimum value in $\dom(\feat(t))$ such that $T_{c_r}$ is valid for $\exex_t[\feat(t)> \thr^R(t)]$.
\end{itemize}

Before formally proving in Lemma~\ref{lem:LR} that can efficiently
compute $\exex_t$, $\thr^L(t)$, and $\thr^R(t)$ for every fixed node
$t$ of $T$, we want to describe the role of $\exex_t$ in the proof
of Lemma~\ref{lem:comp-branch}.


Let us consider the following situation. Suppose we are trying to find
a DT of minimum size for a CI $E$ that uses at least every feature in a
given support set $S$ (but might also use other features).
The first step would be to compute an optimum DT $T^*$ for $E$
such that $\feat(T^*)=S$.
Next we analyse the case an optimum DT for $E$ uses not only every
feature from $S$ but some additional
feature: for this reason we consider DT patterns $T$ of size at most
$s$, where the unknown feature $\QF$ stands a placeholder for other features.
...
Let $E$ be a CI and let $S$ be a support set for $E$ and let $T$ be an
\emph{$S$-DT pattern} of size at most $s$, i.e., $\feat(T)=S\cup \{q\}$.

If $T$ is a valid DT pattern for $E$, then $T$ can easily be turned into a DT
that classifies $E$ by (left or right) contracting every non-fixed
node of $T$.

The following two lemmas cover the case that $T$ is not a valid DT pattern
for $E$.

\begin{lemma}\label{lem:expe}
  Let $T$ be a DT pattern that is not valid for $E$. For every node
  $t$ of $T$ it holds that $T_t$ is not valid for $\exex_t$.
\end{lemma}

\begin{proof}
  Let $T$ be a DT pattern that is not valid for $E$.
  We show this statement in a root-to-leaves fashion: first we show
  the statement holds for the root; then we prove it holds for every
  other node, given the fact it holds for each of its ancestors (or
  its parent).
  Let $r$ be the root of $T$. 
  By definition $\exex_r=E$ and $T_r=T$ and so the statement follows directly from the assumption.

  Let $v$ be the left child of a fixed node $v_p$. By the definition
  of $\thr^L(v_p)$, the DT pattern $T_v$ is not valid for
  $\exex_v=\exex_{v_p}[\feat(v_p)\leq \thr^L(v_p)+1]$. Similarly if $v$ is the
  right child of a fixed node $v_p$, the DT pattern $T_v$ is not valid
  for $\exex_v=\exex_{v_p}[\feat(v_p)> \thr^R(v_p)-1]$.
  
  % todo: check argument again
  Let $v$ be a child of a non-fixed node $v_p$. Suppose for a 
  contradiction that $T_v$ is valid for $\exex_v$. We show that $T_{v_p}$
  is valid for $\exex_{v_p}$ and consequently reaching a contradiction
  with the assumption: any threshold assignment for the fixed nodes of
  $T_v$ that is a witness of the validity of $T_v$ for $\exex_v$ is also
  a threshold assignment for the fixed nodes of $T_{v_p}$ that is a
  witness of the validity of $T_{v_p}$ for $\exex_{v_p}=\exex_v$; note this is
  true because $v_p$ is a non-fixed node.
\end{proof}

\begin{lemma}\label{lem:gap}
  Let $T$ be a DT pattern that is not valid for $E$. For every fixed
  node $v$ of $T$ it holds that $\thr^L(v)<\thr^R(v)$.
...
\begin{proof}
  Let $T$ be a DT pattern that is not valid for $E$. 
  Suppose for a contradiction that there is a fixed node $v^*$ such
  that $\thr^L(v^*)\geq \thr^R(v^*)$. Let $c_{l}$ and $c_r$ be the left
  and right child of $v^*$. We can set the threshold for $\feat(v^*)$
  as $\thr^L(v^*)$ and note that, by definition and the assumption,
  $T_{c_{l}}$ is valid for $\exex_{c_{l}}$ and $T_{c_r}$ is valid
  for $\exex_{c_r}$. This is a contradiction with Lemma~\ref{lem:expe} as
  for every node $v\in V(T)$, $T_v$ is not valid for $\exex_v$.
\end{proof}

Now we are finally ready to prove that we can efficiently compute $\exex_v$,
$\thr^L(v)$ and $\thr^R(v)$ for every node $v\in V(T)$.

\begin{lemma}\label{lem:LR}
  Let $E$ be a CI, let $T$ be a DT pattern of depth at most $d$. Then
  there is an algorithm that runs in time
  $\bigoh(2^{d^2/2}n^{1+\littleoh(1)}\log n)$ and computes the set
  $\exex_v$ and thresholds $\thr^L(v)$ and $\thr^R(v)$ for every node $v\in
  V(T)$.
\end{lemma}
\begin{proof}
  The idea is to use the recursive algorithm {\bf findLR} illustrated
  in Algorithm~\ref{alg:findLR}. That is, given $E$, $T$, the
  algorithm {\bf findLR} attempts to find the triple
  $(\exex_v,\thr^L(v),\thr^R(v))$ for every node $v\in
  V(T)$. Lines~\ref{findLRLS} to~\ref{findLRLE}: if $T$ consists of a
  leaf node, the algorithm just report $(E,\NULL{},\NULL{})$. Let
  $c_{\ell}$ and $c_r$ be the left, risp. right, child of the root
  $v$. Lines~\ref{findLRUFS} to~\ref{findLRUFE}: if the root of $T$ is
  a non-fixed node, the algorithm calls itself recursively to compute
  on $(E,T_{c_\ell})$ and $(E,T_{c_r})$. Lines~\ref{findLRBS}
  to~\ref{findLRSF}: if the root of $T$ is a fixed node $v$, the
  algorithm computes the pair $(t_\ell,t_r)$ for the root using the
  algorithm {\bf binarySearch} and then calls itself recutsively to
  compute the triple for $(E[\feat(v)\leq t_\ell+1],T_{c_\ell})$ and
  $(E[\feat(v)>t_r-1],T_{c_r})$.
  
  A key element for the correctness of {\bf findLR} is the algorithm
  {\bf binarySearch} illustrated in Algorithm~\ref{alg:BS}. Given $E$,
  $T$, $f$, $c_\ell$ and $c_r$, this algorithm computes the pair
  $(t_\ell,t_r)$ for the root of $T$ that has feature $f$. This
  sub-routine performs a standard binary search procedure on the array
  $D$ containing all the values in $D_E(f)$ in ascending order to find
  maximum $t_\ell$ and minimum $t_r$ such that $T_{c_\ell}$ and
  $T_{c_r}$ can be extended to DT for $E[f\leq t_\ell]$ and for
  $E[f>t_r]$ respectively. To achieve this, the sub-routine makes at
  most $log|E|$ calls to {\bf findTH}; note that each of those calls
  is made for a tree of smaller depth. Lines~\ref{BSS}
  to~\ref{return0}: the algorithm finds the maximum $t_\ell$ by
  calling algorithm {\bf findTH} in Line~\ref{BSfindTH}
  repeatedly. Lines~\ref{BST} to~\ref{return1}: the algorithm finds
  the minimum $t_r$ by calling algorithm {\bf findTH} in
  Line~\ref{BSfindTH1} repeatedly.
  

  A sub-routine used for {\bf binarySearch} is the algorithm {\bf
    findTH} illustrated in Algorithm~\ref{alg:findTH}. This algorithm
  is very similar to Algorithm~\ref{alg:findLR} but the output is some
  way much simpler.
  

  The running time of Algorithm~\ref{alg:findLR} can now be obtained
  by multiplying the number of recursive calls to {\bf findLR} with
  the time required for one recursive call. To obtain the number of
  recursive calls first note that if {\bf findLR} is called with DT
  pattern of depth $d$, then it makes at most $(2\log n)+2$ recursive
  calls to {\bf findLR} with a pattern of depth at most $d-1$, where
  $n=|E|$. Therefore the number $T(n,d)$ of recursive calls for a
  pattern of depth $d$ is given by the recursion relation
  $T(n,d)=(2(\log n)+2)T(n,d-1)$ starting with $T(n,0)=0$. This
  implies that $T(n,d)\in \bigoh((\log n)^d)$. Finally, the runtime
  for one recursive call is easily seen to be at most $\bigoh(n\log
  n)$. Hence, the total runtime of the algorithm is at most
  $\bigoh((\log n)^dn\log n)$, which because (see also~\cite[Exercise
  3.18]{CyganFKLMPPS15}):
  \begin{equation*}
...
\begin{algorithm}[h]
\caption{Algorithm to compute the triple $(\exex_v,\thr^L(v),\thr^R(v))$ for every node $v\in V(T)$.}\label{alg:findLR}
\small
\begin{algorithmic}[1]
\INPUT CI $E$, DT pattern $T$
\OUTPUT a triple $(\exex_v,\thr^L(v),\thr^R(v))$ for every node $v\in V(T)$.
...
\begin{algorithm}[h]
\caption{Algorithm to compute the pair $(\thr^L(r),\thr^R(r))$ for the root $r$ of $T$} \label{alg:BS}
\small
\begin{algorithmic}[1]
...
Now we have computed a set $\exex_v$ for every node $v\in V(T)$, whether it is a leaf, fixed or non-fixed node. A {\it pool set} for node $v\in V(T)$ is a set $\Pi(v)\subseteq \exex_v$, such that if $\Pi(v)\subseteq \exex_T(v)$ then either

\begin{itemize}
\item $T_v$ is not valid for $\exex_v$, or
\item for any complete improvement $T_v'$ for $T_v$ that that is valid for $\exex_v$, there are two elements $e,e'\in \Pi(v)$ and there is a non-fixed node $u$ for $T$ such that $\feat_{T'}(u)$ must distinguish $e$ and $e'$.
\end{itemize}

For every node $v\in V(T)$, we define $\Pi(v)$ in a leaves-to-root fashion as follows. If $v$ is a negative leaf then $\Pi(v)=\{e^+\}$, where $e^+$ is any example in $E^+\cap \exex_v$; similarly, if $v$ is a positive leaf then $\Pi(v)=\{e^-\}$, where $e^-$ is any example in $E^-\cap \exex_v$. Let $c_{\ell}$ and $c_r$ be the left, resp. right, child of $v$, then $\Pi(v)=\Pi(c_{\ell})\cup \Pi(c_r)$.
...
\begin{proof}
We show this by induction on the depth of $T$ and let $v$ be the root of $T$. Since $E_T(v)=E$ it is trival to note that $\Pi(v)\subseteq E_T(v)$. We start proving the base case: let $T$ be a pattern of depth 0. Suppose $v$ is negative leaf. Since $\exex_v=E$ is not uniform, there is an example $e^+\in E^+\cap \exex_v$. The case where $v$ is a positive leaf can be proved in a symmetrical manner. 
...
\begin{proof}
Let $E$ be a CI, a support set $S$ for $E$ and an integer $s$. We start by enumerating all DT patterns $T$ of size at most $s$ such that $\feat(T)=S\cup \{q\}$. For every such DT pattern $T$, thanks to Lemma~\ref{lem:LR}, we are able to obtain the set $\exex_v$ for every node $v\in V(T)$ in time $\bigoh(2^{d^2/2}n^{1+\littleoh(1)}\log n)$. In a leaves-to-root fashion, we are able to compute the set $\Pi(v)$ for every node $v\in V(T)$ and ultimately $\Pi(T)$.
...
\item What other (structural) parameters can be exploited to
  ...
  tractable parameterized by the rank-width of $G_I(E)$?
