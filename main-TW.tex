...
In the following, let $E$ be a CI and $q\not\in feat(E)$. A {\it decision tree pattern}, or simply a {\it DT pattern}, $T$ is a rooted subcubic tree, where every leaf-node is either a {\it positive} or {\it negative} leaf and every non-leaf node is labelled with a feature in $feat(E)\cup \{q\}$. Finally we say that a node $v\in V(T)$ is a {\it fixed node} if $\phi(v)\in feat(E)$.

A DT pattern $T^*)$ is an {\it improvement} for a pattern $T$ if $lab_{T}(v)=lab_{T^*}(v)$ for every fixed node $v$ of $T$. 
...
We say that a pattern $(T,\phi)$ can classify $E$ if there exists a complete improvement $(T,\phi^*)$ for $(T,\phi)$ and there exists a threshold assignment $\psi$ for $(T,\phi^*)$ such that $(T,\phi^*)$ classifies $E$ given $\psi$.
...
feature: for this reason we consider patterns $(T,\phi)$ with $T$ of size at most $s$ and such that $Im(\phi)=S\cup \{q\}$.
...
\medskip
\noindent
{\bf Claim 1.} {\it Let $(T,\phi)$ be a pattern. For every node $v\in V(T)$, 
either $(T_v,\phi)$ can not classify every example in $E_v$
or there is a non-fixed node $w$ in $(T_v,\phi)$ such that distinguishes two examples in $E_v$.
.}

\smallskip
\noindent
We prove this claim by induction on the depth of $T$. Let $r$ be the root of $T$. First suppose $(T,\phi)$ is a pattern of depth zero, that is $T$ has only $r$ as node. Since $E$ is not uniform, then $(T_r\phi)$ does not classify every example of $E_r=E$.

Now, let $(T,\phi)$ be a pattern of height at least one and let $\ell$ and $r$ be the left and right child of $r$.
...
\State $\thres_{\ell} \gets$ \Call{findLR}{$E$, $(T_{c_{\ell}},\phi)$}
\State $\thres_r \gets$ \Call{findLR}{$E$, $(T_{c_r},\phi)$}
\If{$\thres_{\ell}\neq$ \NULL{} and $\thres_r\neq$ \NULL{}}
...
\State $f\gets \phi(r)$
\State $(t_\ell,t_r) \gets$ \Call{binarySearch}{$E$, $(T,\phi)$, $f$, $c_{\ell}$, $c_r$}\label{findLRBS}
\State $\thres_\ell \gets$ \Call{findLR}{$E[f\leq t_\ell+1]$, $(T_{c_{\ell}},\phi)$}
\State $\thres_r \gets$ \Call{findLR}{$E[f>t_r-1]$, $(T_{c_r},\phi)$}\label{findLRSF}
...
\small
\begin{algorithmic}[1]
\INPUT CI $E$, pattern $(T,\phi)$, feature $f$ of the root of $T$, left child $c_\ell$ of the root of $T$, right child $c_r$ of the root of $T$
...
\Function{\textbf{binarySearch}}{$E$, $(T,\phi)$, $f$, $c_\ell$, $c_r$}
...
\If{\Call{findTH}{$E[f \leq D[m]]$, $(T_{c_\ell},\phi)$} $=$ \TRUE} \label{BSfindTH}
...
\While{$L \leq  R$}
\State $m \gets \lfloor(L + R) / 2\rfloor$
\If{\Call{findTH}{$E[f > D[m]]$, $(T_{c_r},\phi)$} $=$ \TRUE}\label{BSfindTH1}
...
\begin{algorithmic}[1]
\INPUT CI $E$, pattern $(T,\phi)$
\OUTPUT \TRUE~if $(T,\phi)$ can classify all examples in $E$, \FALSE~otherwise
\Function{\textbf{findTH}}{$E$, $(T,\phi)$}
\State $r \gets$ ``root of $T$''
...
\State $\thres_\ell \gets$ \Call{findTH}{$E$, $(T_{c_\ell},\phi)$}
\State $\thres_r \gets$ \Call{findTH}{$E$, $(T_{c_r},\phi)$}
...
\State $f\gets \phi(r)$
\State $t \gets$ \Call{binarySearch}{$E$, $(T,\phi)$, $f$, $c_\ell$, $c_r$}\label{findTHBS}
\State $\thres_\ell \gets$ \Call{findLR}{$E[f\leq t_\ell+1]$, $(T_{c_{\ell}},\phi)$}
\State $\thres_r \gets$ \Call{findLR}{$E[f>t_r-1]$, $(T_{c_r},\phi)$}\label{findTHSF}
\If{$\thres_r=$~\FALSE}
...
Now we have computed a set $E_v$ for every node $v\in V(T)$, whether it is a leaf, fixed or non-fixed node. A {\it pool set} for node $v\in V(T)$ is a set $\Pi(v)\subseteq E_v$, such that if every example of $\Pi(v)$ arrives at node $v$ then either

\begin{itemize}
\item $(T_v,\phi)$ can not classify $E_v$, or
\item for any complete extension $(T_v,\phi^*)$ for $(T_v,\phi)$ that allow to classify $E_v$, there are two elements $e,e'\in \Pi(v)$ and there is a non-fixed node $u$ for $(T,\phi)$ such that $\phi^*(v)$ must distinguish $e$ and $e'$.
\end{itemize}
...
\smallskip
\noindent
We show this by induction on the depth of $(T,\phi)$. We start proving the base case: let $(T,\phi)$ be a pattern of depth 0. Let $v$ be node of $T$ and suppose it is negative leaf. Since $E_v=E$ is not uniform, there is an example $e^+\in E^+\cap E_v$ and there is no threshold assignment for $T_v$ that would classify $e$. The case $v$ is a positive leaf is similar. 

Now, let $(T,\phi)$ be a pattern of depth at least one and left $v$ root of $T$ with $c_{\ell}$ and $c_r$ as the left and right child.
Suppose first that $v$ is a fixed node and let $f=\phi(v)$. Thanks to Lemma(ADD REFERENCE), for every $e_{\ell}\in \Pi(c_{\ell})$ and $e_r\in \Pi(c_r)$, we know that $f(e_{\ell})<f(e_r)$. This means that either every element of $\Pi(c_{\ell})$ is sent to $c_{\ell}$ or every element of $\Pi(c_r)$ is sent to $c_r$: the statement is proven by induction since $(T_{c_{\ell}},\phi)$ and $(T_{c_r},\phi)$ have smaller depth. 
Finally suppose $v$ is a non-fixed node. Let us consider any complete extension $(T_v,\phi^*)$ of $(T_v,\phi)$. For any threshold possible for $\phi^*(v)$, we have one of the following three cases: every element of $\Pi(c_{\ell})$ is sent to $c_{\ell}$ or every element of $\Pi(c_r)$ is set to $c_r$ or there is an example $e_{\ell}\in \Pi(c_{\ell})$ that ends in $c_r$ and an example $e_r\in \Pi(c_r)$ that ends in $c_{\ell}$. In the first two cases the statement is again proven by induction since $(T_{c_{\ell}},\phi)$ and $(T_{c_r},\phi)$ have smaller depth. In the third case, $v$ is a non-fixed node for $(T,\phi)$ such that $\phi^*(v)$ distinguishes $e_{\ell}$ and $e_r$. This proves Claim 2.
...
We are left to show that $R_0$ is a branching set for $S$. Let $(T,\phi)$ be a minimal DT for $E$ such that $S\subset feat(T)$ and suppose by contradiction that $R_0\cap (feat(T)\setminus S)=\emptyset$. In particular we have that $R(T)\cap (feat(T)\setminus S)=\emptyset$. This means that every non-fixed node of $(T,\phi)$ does not distinguish any two elements in $\Pi(T)$. By Claim 2, $\Pi(T)=\Pi(r)$, where $r$ is the root of $T$, is a pool set and so $(T,\phi)$ can not classify $E$, which is a contradiction.
\end{proof}
