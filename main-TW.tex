...
relabelling node, we first need to introduce the so-called plugin
...
Let $T$ and $T'$ be two DT (templates/skeletons). Let $P=(t,p_1,\dotsc,p_\ell,t')$ be the path
  ...
  $c$. Moreover, $T''$ inherents all feature assignments as well as
  ...
  we can use the plugin operation to reverse the direction or in other
  ...
  $V(T')\setminus V(T)$ with the help of our plugin operation. In
  ...
  $T^0$ after plugin in the path $P(e)$ into $T^0$ at edge $e$,
...
\begin{proof}
  ...
  so we will make use of the plugin operation.
...
\begin{lemma}[relabel node]\label{lem:relabel}
  Let $b\in V(B)$ be relabeling node in $B$. Then $\RRR(b)$ can be
...
\begin{proof}
  ...
  plugin the path $P(e)$ into $T''$ at the edge $(p',a(c))$, where $p'$ is the
  parent of $a(c)$ in $T''$.
